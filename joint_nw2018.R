...
# fitting
res_ana = inla(formula, data = inla.stack.data(stk), family = c("binomial", "binomial"), control.predictor = list(compute = TRUE, A = inla.stack.A(stk)), control.results = list(return.marginals.random = FALSE, return.marginals.predictor = FALSE), control.compute = list(waic = TRUE, dic = TRUE), control.fixed = list(expand.factor.strategy = "inla"))
res_ana2 = inla(formula2, data = inla.stack.data(stk), family = c("binomial", "binomial"), control.predictor = list(compute = TRUE, A = inla.stack.A(stk)), control.results = list(return.marginals.random = FALSE, return.marginals.predictor = FALSE), control.compute = list(waic = TRUE, dic = TRUE), control.fixed = list(expand.factor.strategy = "inla"))
res_ana3 = inla(formula3, data = inla.stack.data(stk), family = c("binomial", "binomial"), control.predictor = list(compute = TRUE, A = inla.stack.A(stk)), control.results = list(return.marginals.random = FALSE, return.marginals.predictor = FALSE), control.compute = list(waic = TRUE, dic = TRUE), control.fixed = list(expand.factor.strategy = "inla"))

res_ana$waic$waic; res_ana$dic$dic #806.5, Inf
res_ana2$waic$waic; res_ana2$dic$dic #805, NaN
...
mesh2 = inla.mesh.2d(boundary = bound2, max.edge = c(0.04, 0.04), cutoff = 0.08/5)
plot(mesh2)
points(c_loc, col = "red", pch = 16, cex = 1) #pdfにする時はcex = 1にする
points(e_loc, col = "green", pch = 16, cex = 1)
mesh2$n #618
...
# コノシロ ------------------------------------
e_fish = mifish %>% filter(name_j == "コノシロ", Depth == "B")
summary(e_fish)
edna = (e_fish$count > 0) + 0

c_fish = data %>% filter(FISH == "konosiro", Y == 2018)
# c_mako = data %>% filter(FISH == "makogarei", Y == 2018, M > 2) #eDNAと月を揃えた方が良い？
summary(c_fish)
catch = (c_fish$CATCH > 0) + 0
summary(catch)
...
# fitting
res_kono = inla(formula, data = inla.stack.data(stk), family = c("binomial", "binomial"), control.predictor = list(compute = TRUE, A = inla.stack.A(stk)), control.results = list(return.marginals.random = FALSE, return.marginals.predictor = FALSE))
summary(res_kono)
...
pred_mean_e = res_kono$summary.fitted.values[index_ep, "mean"]
pred_mean_c = res_kono$summary.fitted.values[index_cp, "mean"]
pred_ll_e = res_kono$summary.fitted.values[index_ep, "0.025quant"]
pred_ul_e = res_kono$summary.fitted.values[index_ep, "0.975quant"]
pred_ll_c = res_kono$summary.fitted.values[index_cp, "0.025quant"]
pred_ul_c = res_kono$summary.fitted.values[index_cp, "0.975quant"]
...
# range_e = apply(coop, 2, range)
proj_e = inla.mesh.projector(mesh2, xlim = range_e[, 1], ylim = range_e[, 2], dims = c(50, 50))
mean_s_ie = inla.mesh.project(proj_e, res_kono$summary.random$i.e$mean)
sd_s_ie = inla.mesh.project(proj_e, res_kono$summary.random$i.e$sd)
...
# マコガレイ ------------------------------------
e_fish = mifish %>% filter(name_j == "マコガレイ", Depth == "B")
summary(e_fish)
edna = (e_fish$count > 0) + 0
summary(edna)

c_fish = data %>% filter(FISH == "makogarei", Y == 2018)
# c_mako = data %>% filter(FISH == "makogarei", Y == 2018, M > 2) #eDNAと月を揃えた方が良い？
summary(c_fish)
catch = (c_fish$CATCH > 0) + 0
summary(catch)
...
# fitting
res_mako = inla(formula, data = inla.stack.data(stk), family = c("binomial", "binomial"), control.predictor = list(compute = TRUE, A = inla.stack.A(stk)), control.results = list(return.marginals.random = FALSE, return.marginals.predictor = FALSE))
summary(res_mako)
...
pred_mean_e = res_mako$summary.fitted.values[index_ep, "mean"]
pred_mean_c = res_mako$summary.fitted.values[index_cp, "mean"]
pred_ll_e = res_mako$summary.fitted.values[index_ep, "0.025quant"]
pred_ul_e = res_mako$summary.fitted.values[index_ep, "0.975quant"]
pred_ll_c = res_mako$summary.fitted.values[index_cp, "0.025quant"]
pred_ul_c = res_mako$summary.fitted.values[index_cp, "0.975quant"]
...
c = coord_fixed(ratio = 1)
s = scale_fill_gradient(name = "encounter prob. (logit)", low = "blue", high = "orange")
g+t+f+c+s+pol+c_map+theme_bw()+labs(title = "makogarei")
...
# range_e = apply(coop, 2, range)
proj_e = inla.mesh.projector(mesh2, xlim = range_e[, 1], ylim = range_e[, 2], dims = c(50, 50))
mean_s_ie = inla.mesh.project(proj_e, res_mako$summary.random$i.e$mean)
sd_s_ie = inla.mesh.project(proj_e, res_mako$summary.random$i.e$sd)
...
# クロダイ ------------------------------------
unique(mifish$name_j)
e_fish = mifish %>% filter(name_j == "クロダイ", Depth == "B")
summary(e_fish)
edna = (e_fish$count > 0) + 0
summary(edna)

unique(data$FISH)
c_fish = data %>% filter(FISH == "kurodai", Y == 2018)
# c_mako = data %>% filter(FISH == "makogarei", Y == 2018, M > 2) #eDNAと月を揃えた方が良い？
summary(c_fish)
catch = (c_fish$CATCH > 0) + 0
summary(catch)
...
# fitting
res_isi = inla(formula, data = inla.stack.data(stk), family = c("binomial", "binomial"), control.predictor = list(compute = TRUE, A = inla.stack.A(stk)), control.results = list(return.marginals.random = FALSE, return.marginals.predictor = FALSE))
summary(res_isi)
...
pred_mean_e = res_isi$summary.fitted.values[index_ep, "mean"]
pred_mean_c = res_isi$summary.fitted.values[index_cp, "mean"]
pred_ll_e = res_isi$summary.fitted.values[index_ep, "0.025quant"]
pred_ul_e = res_isi$summary.fitted.values[index_ep, "0.975quant"]
pred_ll_c = res_isi$summary.fitted.values[index_cp, "0.025quant"]
pred_ul_c = res_isi$summary.fitted.values[index_cp, "0.975quant"]
