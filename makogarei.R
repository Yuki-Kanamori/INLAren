...
# formula
formula = y ~ 0 + eb.0 + cb.0 + f(inla.group(temp), model = "rw1") + f(inla.group(salinity), model = "rw1") + f(inla.group(DO), model = "rw1") + f(inla.group(pH), model = "rw1") + f(i.e, model = spde) + f(i.c, copy = "i.e", fixed = FALSE)
formula2 = y ~ 0 + eb.0 + cb.0 + f(inla.group(temp), model = "rw2") + f(inla.group(salinity), model = "rw2") + f(inla.group(DO), model = "rw2") + f(inla.group(pH), model = "rw2") + f(i.e, model = spde) + f(i.c, copy = "i.e", fixed = FALSE)

# fitting
res_mako = inla(formula, data = inla.stack.data(stk), family = c("binomial", "binomial"), control.predictor = list(compute = TRUE, A = inla.stack.A(stk)), control.results = list(return.marginals.random = FALSE, return.marginals.predictor = FALSE), control.compute = list(waic = TRUE, dic = TRUE))
res_mako2 = inla(formula2, data = inla.stack.data(stk), family = c("binomial", "binomial"), control.predictor = list(compute = TRUE, A = inla.stack.A(stk)), control.results = list(return.marginals.random = FALSE, return.marginals.predictor = FALSE), control.compute = list(waic = TRUE, dic = TRUE))

res_mako$waic$waic; res_mako$dic$dic #2162, 2162 (without cate)
res_mako2$waic$waic; res_mako2$dic$dic #2163, Inf (without cate)
...
pred_mean_e = best_mako$summary.fitted.values[index_ep, "mean"]
pred_mean_c = best_mako$summary.fitted.values[index_cp, "mean"]
pred_ll_e = best_mako$summary.fitted.values[index_ep, "0.025quant"]
pred_ul_e = best_mako$summary.fitted.values[index_ep, "0.975quant"]
pred_ll_c = best_mako$summary.fitted.values[index_cp, "0.025quant"]
pred_ul_c = best_mako$summary.fitted.values[index_cp, "0.975quant"]
...
mean_s_ie = inla.mesh.project(proj_e, best_mako$summary.random$i.e$mean)
sd_s_ie = inla.mesh.project(proj_e, best_mako$summary.random$i.e$sd)
...
# rw1 -----------------------------------------------------------
plot(res_mako$summary.random$`inla.group(temp)`$ID,
     res_mako$summary.random$`inla.group(temp)`$mean,
     type = "l", ylab = "Temp effect", xlab = "Temp", 
     cex.naim = 1.5, cex.lab = 1.5, cex.axis = 1.5, cex = 1.5, lwd = 2)
plot(res_mako$summary.random$`inla.group(salinity)`$ID,
     res_mako$summary.random$`inla.group(salinity)`$mean,
     type = "l", ylab = "Sal. effect", xlab = "Salinity", 
     cex.naim = 1.5, cex.lab = 1.5, cex.axis = 1.5, cex = 1.5, lwd = 2)
plot(res_mako$summary.random$`inla.group(DO)`$ID,
     res_mako$summary.random$`inla.group(DO)`$mean,
     type = "l", ylab = "DO effect", xlab = "DO", 
     cex.naim = 1.5, cex.lab = 1.5, cex.axis = 1.5, cex = 1.5, lwd = 2)
plot(res_mako$summary.random$`inla.group(pH)`$ID,
     res_mako$summary.random$`inla.group(pH)`$mean,
     type = "l", ylab = "pH effect", xlab = "pH", 
     cex.naim = 1.5, cex.lab = 1.5, cex.axis = 1.5, cex = 1.5, lwd = 2)

#not using inla.group
plot(best_mako$summary.random$temp$ID,
     best_mako$summary.random$temp$mean,
     type = "l", ylab = "Temp effect", xlab = "Temp", 
     cex.naim = 1.5, cex.lab = 1.5, cex.axis = 1.5, cex = 1.5, lwd = 2)
plot(best_mako$summary.random$salinity$ID,
     best_mako$summary.random$salinity$mean,
     type = "l", ylab = "Sal. effect", xlab = "Salinity", 
     cex.naim = 1.5, cex.lab = 1.5, cex.axis = 1.5, cex = 1.5, lwd = 2)
summary(e_fish)
plot(best_mako$summary.random$DO$ID,
     best_mako$summary.random$DO$mean,
     type = "l", ylab = "DO effect", xlab = "DO", 
     cex.naim = 1.5, cex.lab = 1.5, cex.axis = 1.5, cex = 1.5, lwd = 2)
plot(best_mako$summary.random$pH$ID,
     best_mako$summary.random$pH$mean,
     type = "l", ylab = "pH effect", xlab = "pH", 
     cex.naim = 1.5, cex.lab = 1.5, cex.axis = 1.5, cex = 1.5, lwd = 2)

effect = rbind(data.frame(x = best_mako$summary.random$`inla.group(temp)`$ID, y = best_mako$summary.random$`inla.group(temp)`$mean, variable = "temp"),
               data.frame(x = best_mako$summary.random$`inla.group(salinity)`$ID, y = best_mako$summary.random$`inla.group(salinity)`$mean, variable = "sal"),
               data.frame(x = best_mako$summary.random$`inla.group(DO)`$ID, y = best_mako$summary.random$`inla.group(DO)`$mean, variable = "do"),
               data.frame(x = best_mako$summary.random$`inla.group(pH)`$ID, y = best_mako$summary.random$`inla.group(pH)`$mean, variable = "ph"))
...
labs = labs(x = "Environmental variable", y = "Effect of environment", title = "makogarei")
g+l+f+labs+theme_bw()
